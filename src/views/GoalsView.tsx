...
function EditGoalSheet({ goal, onClose }: { goal: Goal; onClose: () => void }) {
  ...
  const handleSave = () => {
    ...
    editGoal(goal.id, { title: title.trim(), emoji, category,
        ...
        <div className="overflow-y-auto scroll-area flex-1 px-6 pb-8 space-y-5">
          ...
          <div>
            ...
            <div className="grid grid-cols-3 gap-2">
              {CATEGORIES.map(cat => {
                const m = CATEGORY_META[cat]; const active = category === cat
                return (
...
export default function GoalsView() {
  ...
  return (
    <div className="h-full flex flex-col">
      ...
      <div className="flex-1 scroll-area px-5 pt-2 space-y-3"
        ...
        {goals.length === 0 ? (
          ...
          goals.map((goal, i) => {
            ...
            return (
              <div key={goal.id} className={`rounded-2xl p-4 shadow-sm fade-up fade-up-${Math.min(i+1,5)}`}
                style={{ background: 'var(--white)' }}>
                <div className="flex items-start gap-3 mb-3">
                  <div className="w-12 h-12 rounded-xl flex items-center justify-center text-2xl flex-shrink-0"
                    style={{ background: meta.bg }}>{goal.emoji}</div>
